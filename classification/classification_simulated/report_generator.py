...
class ReportGenerator:
    ...
    def __init__(self, report_queue, stop_event, mode="simul", model_name="default",
        ...
        # Create session folder: {mode}_{model}[_labeled]_{timestamp}
        self.session_start = datetime.now()
        session_ts = self.session_start.strftime("%Y-%m-%d_%H-%M-%S")
        label_suffix = "_labeled" if self.has_label else ""
        self.session_folder_name = f"{mode}_{model_name}{label_suffix}_{session_ts}"
        ...
        # Accuracy tracking for labeled simulation data
        if self.has_label:
    ...
    def _build_table_header(self):
        ...
        # Add "Actual Label" column if this is labeled data
        if self.has_label:
    ...
    def _format_row(self, result):
        ...
        # Add actual label if this is labeled data
        if self.has_label:
    ...
    def _open_minute_file(self, minute_key):
        ...
        self._current_file.write("  NIDS CLASSIFICATION - Minute Report (Simulation)\n")
        self._current_file.write("=" * CLASSIFICATION_REPORT_TABLE_WIDTH + "\n")
        ...
        if self.interface_name:
            self._current_file.write(f"  Source:            {self.interface_name}\n")
        self._current_file.write(f"  Labeled Data:      {'Yes' if self.has_label else 'No'}\n")
    ...
    def _close_minute_file(self):
        ...
        # Show per-minute accuracy if labeled
        if self.has_label and hasattr(self, '_current_minute_accuracy'):
            acc = self._current_minute_accuracy
            if acc["total"] > 0:
                pct = (acc["correct"] / acc["total"]) * 100
                self._current_file.write(f"  Minute Accuracy: {pct:.1f}% ({acc['correct']}/{acc['total']})\n")
    ...
    def _update_stats(self, result):
        ...
        # Accuracy tracking for labeled data
        if self.has_label:
            actual_label = result.get("actual_label")
            if actual_label is not None:
                ...
                # Per-minute accuracy
                if not hasattr(self, '_current_minute_accuracy'):
                    self._current_minute_accuracy = {"correct": 0, "total": 0}
                self._current_minute_accuracy["total"] += 1
                if predicted == actual_label:
                    self._current_minute_accuracy["correct"] += 1
    ...
    def _write_result(self, result):
        ...
        if minute_key != self._current_minute_key:
            # Reset per-minute accuracy tracker
            self._current_minute_accuracy = {"correct": 0, "total": 0}
    ...
    def _write_session_summary(self):
        ...
        with open(summary_path, "w", encoding="utf-8") as f:
            # Session header
            f.write("=" * 100 + "\n")
            f.write("  NIDS CLASSIFICATION - SIMULATION SESSION SUMMARY\n")
            f.write("=" * 100 + "\n\n")
            ...
            if self.interface_name:
                f.write(f"  Source:            {self.interface_name}\n")
            f.write(f"  Labeled Data:      {'Yes' if self.has_label else 'No'}\n")
            ...
            # Always write Threats/Suspicious/Clean
            ...
            # Additionally write Accuracy + Per-Class Precision for labeled
            if self.has_label and "accuracy" in self.stats:
                ...
                if self.stats.get("by_class_accuracy"):
    ...
    def run(self):
        ...
        try:
            while True:
                try:
                    timeout = CLASSIFICATION_QUEUE_TIMEOUT
