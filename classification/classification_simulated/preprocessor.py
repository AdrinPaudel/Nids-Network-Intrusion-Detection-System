...
class Preprocessor:
    ...
    def __init__(self, flow_queue, classifier_queue, stop_event,
        ...
        if model_dir is None:
            if use_all_classes:
                model_dir = os.path.join(PROJECT_ROOT, "trained_models", "trained_model_all")
            else:
                model_dir = os.path.join(PROJECT_ROOT, "trained_models", "trained_model_default")
        self.model_dir = model_dir

        # Load artifacts — sets self.scaler, self.scaler_feature_names,
        # self.selected_features, self.label_encoder (raises on failure).
    ...
    def _preprocess_batch(self, flow_dicts):
        ...
        try:
            ...
            df["Protocol_0"] = np.where(protocol_vals.values == 0, 1, 0)
            df["Protocol_17"] = np.where(protocol_vals.values == 17, 1, 0)
            df["Protocol_6"] = np.where(protocol_vals.values == 6, 1, 0)
