...
class ClassificationSession:
    ...
    def __init__(self, mode="live", interface_name=None, duration=CLASSIFICATION_DEFAULT_DURATION,
                 ...
                 save_flows=False, simul_file_path=None, has_simul_label=False, flows_per_sec=None):
        """
        Args:
            ...
            flows_per_sec: simulation feed rate in flows/sec (None = use config default)
        """
        self.mode = mode
    ...
    def stop(self):
        ...
        Shutdown order (upstream first to let data drain through):
        ...
        7. Set stop_event             (final fallback only if thread doesn't exit)
        8. Print summary
        """
        ...
        # ── Step 1: Stop the flow capture source (no more new flows) ────────
        if self.source:
            self.source.stop()

        # ── Step 2: Inject sentinel so threads know input is finished ──────
        # This is the PRIMARY signal for threads to exit cleanly.
        # Do NOT set stop_event yet — let threads drain via sentinel.
        if self.mode != "batch":
        ...
        # Each thread will naturally drain its input queue and exit.
        ...
        # ── Step 7: Set stop_event as final safety net ────────────────
        # Only use this if a thread is hung. The sentinel should have
        # caused normal shutdown already.
    ...
    def _print_summary(self):
        ...
        if self.mode == "batch":
            ...
            if self.report_generator:
                print(f"  Report folder:     {self.report_generator.report_path}")
            # Emit machine-readable final count AFTER pipeline drain so the
            # route parser can use the true post-drain classified count.
            if self.mode == "simul" and self.classifier:
                print(f"[SESSION] FINAL Classified: {self.classifier.classified_count}")

        print(f"{COLOR_CYAN}{'='*80}{COLOR_RESET}\n")


def resolve_interface_from_input(user_input: str) -> Optional[str]:
    """
    Resolve user input to an interface name.
    Supports:
      - Direct name match: 'eth0', 'wlan0', 'en0'
      - Keyword match: 'wifi', 'ethernet' (auto-selects first)
      - Partial description match: 'Intel', 'Realtek'
    ...
    for iface in interfaces:
        if iface['name'].lower() == user_lower:
            return iface['name']

    # 2. Keyword match: 'wifi' → first WiFi adapter
    if user_lower == 'wifi':
        wifi_ifaces = get_wifi_interfaces(interfaces)
        if wifi_ifaces:
            return wifi_ifaces[0]['name']

    # 3. Keyword match: 'ethernet' → first Ethernet adapter
    if user_lower == 'ethernet':
        eth_ifaces = get_ethernet_interfaces(interfaces)
        if eth_ifaces:
            return eth_ifaces[0]['name']

    # 4. Partial description match
    for iface in interfaces:
        if user_lower in iface['description'].lower():
            return iface['name']

    # 5. Partial name match (as fallback)
    for iface in interfaces:
        if user_lower in iface['name'].lower():
            return iface['name']
...
Examples:
  ...
  python classification.py --live --interface wifi            # Live on WiFi (auto-select first)
  python classification.py --live --interface ethernet        # Live on Ethernet (auto-select first)
  python classification.py --live --interface eth0            # Live on specific interface
  ...
  python classification.py --list-interfaces                  # List network interfaces and exit
    ...
    parser.add_argument(
        "--flow-rate", type=int, default=None,
        help="Simulation flow feed rate in flows/sec (overrides config default)"
    )

    # ── Live mode specific options ──────────────────────────────
    parser.add_argument(
        "--interface", type=str, default=None,
        help="Network interface name (e.g., 'eth0', 'wifi', 'ethernet'). If 'wifi' or 'ethernet', auto-selects first match. Skips interactive menu."
    )
    ...
    if args.simul:
        ...
        # Live mode - interface selection (direct or interactive)
        if args.interface:
            # User provided --interface flag
            interface_name = resolve_interface_from_input(args.interface)
            if interface_name is None:
                print(f"{COLOR_RED}[MAIN] Interface not found or invalid: {args.interface}{COLOR_RESET}")
                print(f"{COLOR_YELLOW}  Try one of:{COLOR_RESET}")
                print(f"{COLOR_YELLOW}    - 'wifi' (auto-select first WiFi)")
                print(f"{COLOR_YELLOW}    - 'ethernet' (auto-select first Ethernet)")
                print(f"{COLOR_YELLOW}    - Device name like 'eth0', 'wlan0', 'en0'")
                print(f"{COLOR_YELLOW}    - Partial description like 'Intel', 'Realtek'{COLOR_RESET}")
                print(f"{COLOR_YELLOW}  Or list available interfaces:{COLOR_RESET}")
                print(f"{COLOR_CYAN}    python classification.py --list-interfaces{COLOR_RESET}\n")
                sys.exit(1)
            print(f"{COLOR_GREEN}[MAIN] Using interface: {interface_name}{COLOR_RESET}")
        else:
