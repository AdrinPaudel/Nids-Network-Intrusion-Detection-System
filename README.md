...
## ⚠️ CRITICAL: Attacks Must Be Launched From Linux

**The NIDS model was trained on attack traffic generated by Linux (Kali). If you attack from Windows, the model will classify attacks as Benign (2nd choice shows correct attack type).**

- ✅ **Attacks from Linux** → Correctly classified (e.g., DoS → DoS)
- ❌ **Attacks from Windows** → Misclassified as Benign (but attack type visible as 2nd choice)

**See [setup/setup_attacker/](setup/setup_attacker/) for Linux attack setup.**
